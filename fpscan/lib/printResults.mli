...
module Make (Res: Results.Register) : sig
  ...
  val print : ?ast_nodes:(Name.t * Ast.stm) list -> 
    ?env_nodes:(Name.t * (Ast.Var.Set.t * Ast.Var.t list * Ast.Var.t list * (Name.t * (Ast.expr * Ast.Var.t) list) list )) list -> 
    ?map:(Name.t * Name.t) Name.Map.t -> Res.t -> Ast.Var.Set.t -> Ast.stm -> Format.formatter -> unit

  (** Same as [print] but only print loop invariants (in the same
      order that the [while] keywords appear in the code). *)
  val print_invariants : Res.t -> Ast.Var.Set.t -> Ast.stm -> Format.formatter -> unit

  val get_unrolled_info: ?node_name: Name.t option -> Res.t -> Location.t -> (int * Res.Dom.t) list
end
