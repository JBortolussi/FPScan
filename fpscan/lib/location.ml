...
let fprint ff loc =
  ...
  in
  (* Format.fprintf ff " uid=%i" loc.loc_cpt; *)
  ()
  
let get_line loc = loc.loc_line
...
let to_string loc =
  if loc.loc_line = loc.loc_line_end then
    ...
    Format.sprintf "File \"%s\", lines %d-%d, characters %d-%d"
    loc.loc_file loc.loc_line loc.loc_line_end (loc.loc_start + 1) loc.loc_line_end_c
