...
(* Profile a unit function
   Parameters :
    * f the function to profile
   Returns a pair (result of the function, execution time in seconds)
 *)
let profile f =
...
(* Take a string and return the number of lines in the string *)
...
let with_out_ch output_filename f =
  ...
  let out_fmt = Format.formatter_of_out_channel out_ch in
  let res = f out_fmt in
  begin
    match output_filename with
    | None -> ()
    | Some _ ->
       Report.nlogf 1 "Results written to %s."
         (output_filename_string output_filename);
...
let fold_left' proj f l =
  match l with
    [] -> assert false
  | [hd] -> proj hd
  | hd::tl  -> List.fold_left (fun accu elem -> f accu (proj elem)) (proj hd) tl
