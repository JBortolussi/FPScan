...
module CheckNonRel (D: NonRelational.Domain) =
  struct
    ...
    let single_int_vals () = (* TODO : add a few more random values *)
      List.map Q.of_int [0; 1; -1; 42; -42; 126456465; 3121321; -53; -331131; min_int; max_int]

    let single_real_vals () =
      if D.base_type = Ast.RealT then
        List.map (fun (x,y) -> Q.of_ints x y) [(1,10);(1,3);(1,100000)]
      else
        []
      ...
      let ok_single, single = 
        List.fold_left (
            ...
            if not (List.mem_assoc set accu) then
              ...
              ok, accu
          ) (true,[]) ((single_int_vals ())@(single_real_vals ()))
      in
